...
contract BrickBond is Brick, ERC721 {
  ...
  mapping (uint => address[]) public _brickApprovalRequests;
  ...
  function requestApproval(uint256 _tokenId) external {
    _brickApprovalRequests[_tokenId].push(msg.sender);
  }
  
  function getBrickRequests(uint256 _brickId) external view returns(address[] memory) {
    return _brickApprovalRequests[_brickId];
  }
